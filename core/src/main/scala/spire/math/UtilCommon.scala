...
object UtilCommon {
  ...
  // Multiplication
  // See Multidigit Multiplication For Mathematicians by Daniel J. Bernstein
  // (http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.20.8270).
  final def shiftLeft(arr: Array[Int], by: Int): Array[Int] = {
    if (by == 0)
      arr
  ...
  final def shiftRight(arr: Array[Int], by: Int): Array[Int] = {
    if (by == 0)
      arr
  ...
  final def sameArrayContents(a: Array[Int], b: Array[Int]): Boolean = {
    val aLen = a.length
    if (aLen != b.length)
      return false
    var i = 0
